...
pub fn build(b: *std.Build) void {
    ...
    const add_mod = b.createModule(.{
        .root_source_file = b.path("src/cli/add.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "sqlite", .module = sqlite_mod },
    ...
    const add_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("tests/add_test.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "sqlite", .module = sqlite_mod },
    ...
    const write_mod = b.createModule(.{
        .root_source_file = b.path("src/db/write.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "sqlite", .module = sqlite_mod },
            .{ .name = "database", .module = db_mod },
        },
    });

    // Write Performance Tests
    const write_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("tests/write_test.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "sqlite", .module = sqlite_mod },
                .{ .name = "db", .module = db_mod },
                .{ .name = "write", .module = write_mod },
            },
        }),
    });
