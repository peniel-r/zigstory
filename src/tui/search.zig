...
pub const SearchState = struct {
    ...
    pub fn performSearch(
        ...
        // Use simple LIKE query - bypasses FTS5 entirely
        // GROUP BY cmd to deduplicate, showing most recent occurrence
        const query_sql =
        ...
        var iter = try stmt.iterator(QueryRow, .{
            .pattern = like_pattern.items,
            .lim = @as(i64, @intCast(limit)),
        });
